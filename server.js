...
app.post('/add', (req,res) => { // 1. 폼에다가 누군가 입력을하면
    res.send('전송완료') 
    db.collection('counter').findOne({name : "게시물갯수"},function(에러,결과){// db에 counter 이라는 게시물을 찾고
        console.log(결과.totalPost);
        var 총게시물갯수 = 결과.totalPost; //게시물갯수라는 이름을 갖는 total값을가져온다.

        db.collection('post').insertOne({_id: 총게시물갯수 +1, title: req.body.title , date: req.body.date}, function(에러,결과){
            console.log('데이터 저장완료!!!');
            // +각 아이디값을 부여 해주려면 총게시물갯수 +1을 해주어야한다.
            db.collection('counter').updateOne({name:'게시물갯수'},{ $inc : {totalPost:1} },function(){
                console.log('1이 증가되고있습니다.')
            });
        });  
    });
});
